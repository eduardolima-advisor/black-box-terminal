...
# ==============================================================================
# FUNÇÕES DE OTIMIZAÇÃO — nível de módulo, junto aos outros helpers
# Correções aplicadas sem alterar visual ou lógica de negócio
# ==============================================================================
...
def optimize_portfolio(ativos_selecionados, weights=None):
    ...
    # CORREÇÃO 1: yf.download com 1 ticker retorna Series — normaliza para DataFrame
...
def get_monte_carlo_var(lista_ativos, weights, n_sims=10000, confidence=0.95):
    df = yf.download(lista_ativos, start="2020-01-01", progress=False)['Close']
    ...
    avg_rets   = returns.mean().values * 252

    L = np.linalg.cholesky(cov_matrix + np.eye(len(cov_matrix)) * 1e-8)

    random_numbers    = np.random.normal(0, 1, (len(returns.columns), n_sims))
    simulated_returns = (avg_rets[:, np.newaxis] / 252) + (L @ random_numbers / np.sqrt(252))

    portfolio_simulations = weights @ simulated_returns
    var_mc = np.percentile(portfolio_simulations, (1 - confidence) * 100)

    return {"var_mc": var_mc, "simulacoes": portfolio_simulations}


def optimize_portfolio_cvar(tickers, target_return=None, span=180):
    ...
    tickers_presentes = [t for t in tickers if t in data.columns]
    if not tickers_presentes:
        return None
    data    = data[tickers_presentes]
    returns = data.pct_change().dropna()

    try:
        from pypfopt import expected_returns as er, risk_models
        from pypfopt.efficient_frontier import EfficientCVaR

        mu = er.ema_historical_return(data, span=span)
        S  = risk_models.CovarianceShrinkage(data).ledoit_wolf()

        ec = EfficientCVaR(mu, returns, beta=0.95)
        ...
        clean_w = ec.clean_weights()

        clean_w = {k: v for k, v in clean_w.items() if k in tickers_presentes}
        total_w = sum(clean_w.values())
        if total_w > 0:
            clean_w = {k: round(v / total_w, 4) for k, v in clean_w.items()}

        return {
            "weights":       clean_w,
            "expected_cvar": ec.portfolio_performance()[1],
            "returns_df":    returns
        }

    except Exception:
        # Plano B: SLSQP robusto
        n    = len(tickers_presentes)
        ...
        raw_weights = np.clip(res.x, 0, 1)
        raw_weights /= raw_weights.sum()  # renormaliza após clip

        return {
            "weights":       dict(zip(tickers_presentes, np.round(raw_weights, 4))),
            "expected_cvar": float(res.fun),
            "returns_df":    returns
        }
...
# --- 3. TELA: PAINEL ALOCAÇÃO (Conectada pelo ELIF) ---
    ...
    def extrair_dados_suitability(file):
        ...
        try:
            ...
            return {"perfil": "ERRO", "horizonte": "Falha na Leitura", "objetivo": str(e)}

        dados = {"perfil": "Moderado", "horizonte": "Não Identificado", "objetivo": "Preservação"}

        if texto and "PERFIL IDENTIFICADO:" in texto:
            perfil_raw = texto.split("PERFIL IDENTIFICADO:")[1].split("\n")[0].strip().upper()
        ...
        if texto and "HORIZONTE TEMPORAL DE INVESTIMENTO" in texto:
            try:
                raw_h = texto.split("HORIZONTE TEMPORAL DE INVESTIMENTO")[1].split("R:")[1].split("\n")[0]
                dados["horizonte"] = raw_h.strip()
            except: pass

        if texto and "OBJETIVO PRIMÁRIO DE INVESTIMENTO" in texto:
            try:
                raw_o = texto.split("OBJETIVO PRIMÁRIO DE INVESTIMENTO")[1].split("R:")[1].split("\n")[0]
                dados["objetivo"] = raw_o.strip()
            except: pass

        return dados

    # --- CSS DO FILE UPLOADER ---
    st.markdown("""
        <style>
            [data-testid="stFileUploader"] { padding: 0px; }
            [data-testid="stFileUploader"] section {
                padding: 0px; background-color: transparent;
                border: 1px dashed #262626; min-height: 40px;
            }
            [data-testid="stFileUploader"] section > div { display: none; }
            [data-testid="stFileUploader"] button {
                font-size: 0.6rem !important; background-color: transparent !important;
    ...
    if 'ai_purpose' not in st.session_state:
        with area_upload.container():
            suitability_file = st.file_uploader(
                "UPLOAD IPS", type=['pdf', 'txt'],
                key="suit_mini", label_visibility="collapsed"
            )
    ...
    with c1:
        ...
        st.markdown(f"""
            <div style='border-left: 2px solid #C5A059; padding-left: 10px; margin-bottom: 5px;'>
                <p style='margin:0; font-size:0.6rem; color:#64748b; text-transform:uppercase; font-weight:700; letter-spacing:0.5px;'>Propósito Estratégico</p>
                <p style='margin:0; font-size:0.85rem; color:#D8D8D8; font-weight:500; text-transform:uppercase;'>{purp}</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    with c2:
        ...
        st.markdown(f"""
            <div style='border-left: 2px solid #C5A059; padding-left: 10px; margin-bottom: 5px;'>
                <p style='margin:0; font-size:0.6rem; color:#64748b; text-transform:uppercase; font-weight:700; letter-spacing:0.5px;'>Horizonte</p>
                <p style='margin:0; font-size:0.85rem; color:#D8D8D8; font-weight:500; text-transform:uppercase;'>{horiz}</p>
            </div>
        """, unsafe_allow_html=True)

    with c3:
        risk_raw = st.session_state.get('ai_risk', "MODERADO").upper()
        bg_color = risk_color_map.get(risk_raw, "#808080")
        st.markdown(f"""
            <div style='text-align:right;'>
                <p style='margin:0; font-size:0.6rem; color:#64748b; text-transform:uppercase; font-weight:700; letter-spacing:0.5px; margin-bottom:4px;'>Risco Extraído</p>
                <span style='background-color:{bg_color}; color:black; padding:3px 12px; border-radius:1px; font-size:0.65rem; font-weight:800; font-family: "Courier New", monospace;'>{risk_raw}</span>
            </div>
        """, unsafe_allow_html=True)
    ...
    c_add, _ = st.columns([0.5, 0.5])
    with c_add:
        capital_raw = st.text_input(
            label="Add",
            placeholder="＋ INSERIR CAPITAL ALOCÁVEL (Ex.: 100000)",
            label_visibility="collapsed",
            key="input_capital_alocavel"       # ← CORREÇÃO 1: key única (não conflita com input_new_ac da seleção de ativos)
        )

    # --- LÓGICA DE INTEGRAÇÃO (SAA) — calculada uma única vez ---
    risk_map = {
    ...
    with k2:
        st.markdown(f"""
            <div class="stat-card" style="border-top: 2px solid #C5A059;">
                ...
                <p class="stat-value">{old_equity:.1%}</p>
                <p class="stat-status" style="color: #808080;">ESTRATÉGIA</p>
            </div>
        """, unsafe_allow_html=True)

    with k3:
        status_color_k3 = "#C5A059" if v_acoes > 0 else "#666666"   # ← CORREÇÃO 2: variável renomeada para não sombrear status_color do bloco seguinte
        st.markdown(f"""
            <div class="stat-card" style="border-top: 2px solid {status_color_k3};">
                <p class="stat-label">Exposição em Renda Variável</p>
                <p class="stat-value">R$ {v_acoes:,.0f}</p>
                <p class="stat-status" style="color: {status_color_k3};">RISCO NOMINAL</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    with ca:
        w_eq = st.number_input(
            "Equity Allocation (%)",
            0, 100,
            int(old_equity * 100),
            key=f"eq_focus_{perfil_extraido}"
        ) / 100

    with cb:
        weights_others = 1.0 - w_eq
        st.number_input(
            "Other Assets (%)",
            0, 100,
            int(weights_others * 100),
            key="others_focus",
            disabled=True
        )
        w_others = weights_others  # ← CORREÇÃO 3: valor vem do cálculo, não do widget disabled (que retorna None em algumas versões do Streamlit)
    ...
    with col_b:
        fig = go.Figure(data=[go.Pie(
            ...
            hole=.78,
            marker=dict(colors=['#C5A059', '#1A1A1A'], line=dict(color='#080808', width=2)),
    ...
    try:
        ...
        nota = (
            f"A exposição de {txt_perc} em Equities (Brasil & Internacional) reflete o mandato de risco '{perfil_nome}'. "
            f"O aporte nominal de {txt_exposicao} deve ser alocado conforme as diretrizes da BLACK BOX Capital, "
            f"cuja tese integra modelagem estocástica e otimização de portfólio, "
            f"que garantem aderência rigorosa da distribuição dos ativos aos limites de volatilidade dos mercados doméstico e global."
        )
        ...
        pdf.set_font("Helvetica", 'I', 7)
        pdf.set_text_color(150, 150, 150)
        pdf.multi_cell(page_width, 4,
                       clean_pdf("Confidencial | BLACK BOX. Elaborado de acordo com padrões institucionais. Riscos de mercado inerentes."),
                       align='C')
        ...
        with col_central:
            st.download_button(
                label="GERAR RELATÓRIO INSTITUTIONAL (.PDF)",
                data=pdf_bytes,
                file_name=f"BLACK_BOX_Strategy_{datetime.now().strftime('%Y%m%d')}.pdf",
                mime="application/pdf",
                use_container_width=True,
                type="primary"
            )
...
# ==============================================================================
# BLOCO DA PÁGINA painel_otimizacao — seção de seleção de ativos
# Visual idêntico ao original; apenas a lógica do botão RUN foi corrigida
# ==============================================================================
    ...
    with st.container():
        ...
        with c_add:
            ...
            if novo_ticker and novo_ticker not in st.session_state.lista_acoes_din:
                st.session_state.lista_acoes_din = sorted(
                    st.session_state.lista_acoes_din + [novo_ticker]
                )
        ...
        if ativos_selecionados:
            ...
            ai_weights = st.session_state.get('portfolio_weights', {})

            for ativo in ativos_selecionados:
                c_label, c_input, c_rem = st.columns([1, 4, 0.3])

                # CORREÇÃO 4 (UI): converte peso para float seguro antes de passar ao widget
                default_weight = float(ai_weights.get(ativo, 0.0))
                default_weight = max(0.0, min(1.0, default_weight))  # clamp [0, 1]

                with c_label:
                    st.markdown(
                        f"<p style='font-size:0.85rem; color:#E2E8F0; margin-top:8px;'>{ativo}</p>",
                        unsafe_allow_html=True
                    )
                with c_input:
                    p = st.number_input(
    ...
    if ativos_selecionados:
        ...
        with c_btn:
            if st.button("RUN SMART ALLOCATION", key="run_weights", use_container_width=True, type="primary"):
                with st.spinner("LYNX ENGINE: EXECUTING MODEL..."):
                    resultado = optimize_portfolio_cvar(ativos_selecionados)

                    if resultado and resultado.get('weights'):
                        st.session_state['portfolio_weights'] = resultado['weights']

                        # Monte Carlo usa apenas os tickers que retornaram com peso
                        tickers_validos = list(resultado['weights'].keys())
                        pesos_lista     = list(resultado['weights'].values())

                        if tickers_validos and sum(pesos_lista) > 0:
                            mc_audit = get_monte_carlo_var(tickers_validos, pesos_lista)
                            st.session_state['mc_var_results'] = mc_audit['var_mc']

                        st.toast("OPTIMIZATION CONCLUDED: 10K ITERATIONS VALIDATED.")
                        time.sleep(1)
                        st.rerun()
                    else:
                        st.error("Erro no download de dados. Verifique os tickers.")
        ...
        try:
            with st.spinner('⏳ CALCULANDO MÉTRICAS DE MERCADO...'):
                ...
                with tab_overview:
                    ...
                    color_border = color_equity if 0.99 <= total_acoes <= 1.01 else "#800000"
                    ...
                    with c1:
                        st.markdown(f'<div class="stat-card" style="border-top: 3px solid {color_border};"><p class="stat-label">EQUITY EXPOSURE</p><p class="stat-value">{total_acoes:.1%}</p></div>', unsafe_allow_html=True)
                    ...
                    with col_b:
                        ...
                        if ativos_selecionados and sum(pesos_acoes) > 0:
                            labels = ativos_selecionados
                            values = pesos_acoes
                            
                            # Gerando tons de dourado para as ações para manter o estilo
                            # (Quanto mais ações, mais tons ele cria automaticamente)
                            colors = [f'rgba(197, 160, 89, {1 - (i * 0.15)})' for i in range(len(ativos_selecionados))]
                        else:
                            # Estado padrão caso nada esteja selecionado
                            labels = ['Equities']
                            values = [0]
                            colors = ['#C5A059']
                        ...
                        fig.add_annotation(
                            text=f"TOTAL EQUITY<br><br><b style='font-size:22px; color:#E0E0E0;'>{sum(pesos_acoes):.1%}</b>",
                            showarrow=False, x=0.5, y=0.5,
                            font=dict(size=14, family="Inter", color="#808080"),
                            align="center"
                        )
